...
## IF Prescreen ==1 means vaccinating only seropositive == 0 means vaccinate all with any serostatus
prescreening = 1

# prescreening_non_sero = 1 ## for creating scenario for pre-screening but setting (note in this case vaccine coverage will be
                          ##  will be replaced by test coverage)

# test_coverage <- 0.80 ## 80% will people will be screened and vaccinated if they are found to be seropositive
...
## after 5 years it will stop waning
n_year_waning <- 2

## 4.5 year with the reported efficacy then each year wanes at a rate "waning"

## TODO:  
vac_stage_rate <- c( 1/(54*30), rep( (1/(12*30)), (n_year_waning-1)), 0)    #c( rep(1/(12*30),3), 0)

n_vac_stage <- length(vac_stage_rate)

n_sero <- 4

## rate of waning per year from baseline values of VE
waning <- 0.0


##################
## efficcay estimates for Dengvaxia 
ve_inf_sero_n_low = 0 # 0.05 #-0.359
...
## indices 
  ...
  tot_pop_age_yearly <- calc_year_age_wo_sum(array(out[ind_tot_age,,], dim = c(n_age, length(t))))      ## yearly pop. in each group
