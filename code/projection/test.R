  ...
  for (i in 1:n_vac_stage) {
    for (j in 1:n_sero) {
      ...
      if (j==3|j==4) {
        ...
        effi_symp_sero_n_array[i,j] <- 1 - ((1 - runif(n = 1, 
                                                       min = symp_low_sero_n[i,j] , 
                                                       max = symp_up_sero_n[i,j] ) )/(1 - effi_inf_sero_n_array[i]))
      }
      
      
      effi_symp_sero_p_array[i,j] <- 1 - ((1 - runif(n = 1, 
                                                     min = symp_low_sero_p[i,j] , 
                                                     max = symp_up_sero_p[i,j] ) )/(1 - effi_inf_sero_p_array[i]))
...
get_post_sample <- function(k) {
  ...
  rho_2_a1 = rho_2_sample[1]
  rho_2_a2 = rho_2_sample[2]
  rho_2_a3 = rho_2_sample[3]
  rho_2_a4 = rho_2_sample[4]
  rho_2_a5 = rho_2_sample[5]
  rho_2_a6 = rho_2_sample[6]
  rho_2_a7 = rho_2_sample[7]
  rho_2_a8 = rho_2_sample[8]
...
n_vac_coverage = length(vac_coverage)
...
par <- list(
  ...
  plot(tot_pop_age_v_yearly[25,]/tot_pop_age_yearly[25,])

  plot(colSums(tot_pop_age_yearly))
