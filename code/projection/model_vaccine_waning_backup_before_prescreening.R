...
elig_vac_age_scrng[] <- (1 - test_sensiti)*S_all[i] + test_sensiti*sum(C[i,]) + test_sensiti*sum(S[i,]) + test_sensiti*R[i]
...
vac_Sall_scrng[1] <- if (as.integer(t) == vac_start) (1 - test_sensiti)*vac_switch[i]*vac_coverage*N_age[i]*S_all[i]/elig_vac_age_scrng[i] else
  (1 - test_sensiti)*vac_switch[i]*(lambda_hum*N*N_v_age[i]/N_age[i])*(S_all[i]/elig_vac_age_scrng[i])

vac_Sall_scrng[2:n_age] <- if (as.integer(t) == vac_start) (1 - test_sensiti)*vac_switch[i]*vac_coverage*N_age[i]*S_all[i]/elig_vac_age_scrng[i] else
  (1 - test_sensiti)*vac_switch[i]*(age_rate[i - 1]*(N_age[i - 1]*N_v_age[i] - N_v_age[i - 1]*N_age[i])/N_age[i])*(S_all[i]/elig_vac_age_scrng[i])

vac_C_scrng[1,1:n_sero] <- if (as.integer(t) == vac_start) (test_sensiti)*vac_switch[i]*vac_coverage*N_age[i]*C[i,j]/elig_vac_age_scrng[i] else
  (test_sensiti)*vac_switch[i]*(lambda_hum*N*N_v_age[i]/N_age[i])*(C[i,j]/elig_vac_age_scrng[i])

vac_C_scrng[2:n_age,1:n_sero] <- if (as.integer(t) == vac_start) (test_sensiti)*vac_switch[i]*vac_coverage*N_age[i]*C[i,j]/elig_vac_age_scrng[i] else
  (test_sensiti)*vac_switch[i]*(age_rate[i - 1]*(N_age[i - 1]*N_v_age[i] - N_v_age[i - 1]*N_age[i])/N_age[i])*(C[i,j]/elig_vac_age_scrng[i])

vac_S_scrng[1,1:n_sero] <- if (as.integer(t) == vac_start) (test_sensiti)*vac_switch[i]*vac_coverage*N_age[i]*S[i,j]/elig_vac_age_scrng[i] else
  (test_sensiti)*vac_switch[i]*(lambda_hum*N*N_v_age[i]/N_age[i])*(S[i,j]/elig_vac_age_scrng[i])

vac_S_scrng[2:n_age,1:n_sero] <- if (as.integer(t) == vac_start) (test_sensiti)*vac_switch[i]*vac_coverage*N_age[i]*S[i,j]/elig_vac_age_scrng[i] else
  (test_sensiti)*vac_switch[i]*(age_rate[i - 1]*(N_age[i - 1]*N_v_age[i] - N_v_age[i - 1]*N_age[i])/N_age[i])*(S[i,j]/elig_vac_age_scrng[i])

vac_R_scrng[1] <-  if (as.integer(t) == vac_start) (test_sensiti)*vac_switch[i]*vac_coverage*N_age[i]*R[i]/elig_vac_age_scrng[i] else
  (test_sensiti)*vac_switch[i]*(lambda_hum*N*N_v_age[i]/N_age[i])*(R[i]/elig_vac_age_scrng[i])

vac_R_scrng[2:n_age] <- if (as.integer(t) == vac_start) (test_sensiti)*vac_switch[i]*vac_coverage*N_age[i]*R[i]/elig_vac_age_scrng[i] else
  (test_sensiti)*vac_switch[i]*(age_rate[i - 1]*(N_age[i - 1]*N_v_age[i] - N_v_age[i - 1]*N_age[i])/N_age[i])*(R[i]/elig_vac_age_scrng[i])
