...
## indices 
  ...
  # inf_age_v_primary_yearly <- calc_year_age(inf_age_sero_v_primary <- array(out[ind_inf_v_primary,,], dim = c(n_age, n_sero, length(t)))) 
...
for (i in 1:length(v_coverage)) {
  ...
  out_parallel <- foreach(j =  1:n_vac_age, .combine = rbind, .packages = c("dplyr", "tidyr", "here", "odin.dust", "sn")) %dopar% {
    ...
    for (k in 1:n_sample) {
      ...
      par <- list(
        ...
        effi_symp_sero_n = effi_symp_sero_p,
  ...
  saveRDS(list(coverage = v_coverage, 
               ...
               v_age_list = v_a,
               output = results_list), 
          file = here::here("model_output", paste0("same_serop_sero_n_import_a7_sero_dom_", sero_domin, "_vcov_", v_coverage,"_baseline_", baseline,
                                                   "_nsamp_", n_sample,"_waning_", waning, ".rds" )))
